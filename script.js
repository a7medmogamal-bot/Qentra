// ============ Configuration ============
const OPENROUTER_API_KEY = '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';
...
async function callQentraAPI(imageBase64, prompt) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error('خطأ ' + response.status + ': ' + (errorData.error?.message || 'Unknown'));
        }
...
async function chatWithQentra(message) {
    try {
        const payload = {
            model: OPENROUTER_MODEL,
            messages: [
                { role: 'system', content: 'You are QENTRA AI, an educational assistant. Answer in the same language as the user.' },
