// ============ Configuration ============
const GEMINI_API_KEY = 'AQ.Ab8RN6LYAk7rh9d-0v_XC6dnBHIMxEeRRPttbi57ivgwzlobyg';
...
const translations = {
    ar: {
        ...
        api_key_invalid: 'مفتاح API غير صالح. تحقق من المفتاح وحاول مرة أخرى.',
        rate_limit: 'تم تجاوز الحد اليومي للطلبات. حاول غداً.',
        no_response: 'لم يتم الحصول على رد من الخادم. حاول مرة أخرى.'
    },
    en: {
        ...
        api_key_invalid: 'Invalid API key. Check your key and try again.',
        rate_limit: 'Daily request limit exceeded. Try tomorrow.',
        no_response: 'No response from server. Try again.'
    }
};
    ...
    try {
        ...
        if (data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts) {
            ...
            if (text && text.length > 0) {
                console.log('Success! Response received, length:', text.length);
                return text;
            }
        }
...
async function generateQuestions() {
    ...
    try {
        ...
        switch(error.message) {
            case 'BAD_REQUEST':
                errorMessage = 'الصورة غير مقبولة. جرب صورة أوضح.';
                break;
            case 'FORBIDDEN':
                errorMessage = t('api_key_invalid');
                break;
            case 'MODEL_NOT_FOUND':
                errorMessage = 'النموذج غير متوفر. جرب نموذج آخر.';
                break;
            case 'RATE_LIMIT':
                errorMessage = t('rate_limit');
                break;
            case 'NO_RESPONSE':
                errorMessage = t('no_response');
                break;
            default:
