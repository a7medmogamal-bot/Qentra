// ============ Configuration ============
const AI_API_KEY = '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';
const AI_IMAGE_MODEL = 'nlpconnect/vit-gpt2-image-captioning';
const AI_TEXT_MODEL = 'microsoft/Phi-3-mini-4k-instruct';
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(
                'https://api-inference.huggingface.co/models/' + AI_IMAGE_MODEL,
                {
                    method: 'POST',
                    headers: {
            ...
            if (!response.ok) {
                throw new Error('خطأ ' + response.status + ': ' + (data.error || 'Unknown'));
            }
...
Generate EXACTLY 10 educational questions about this content.
Rules:
- Questions 1-3: Easy
- Questions 4-7: Medium
- Questions 8-10: Hard
- Questions must be deep and meaningful, not superficial
...
{"success": false, "error": "unclear"}`;
            ...
            const response = await fetch(
                'https://api-inference.huggingface.co/models/' + AI_TEXT_MODEL,
                {
                    method: 'POST',
                    headers: {
                        'Authorization': 'Bearer ' + AI_API_KEY,
    ...
    try {
        const response = await fetch(
            'https://api-inference.huggingface.co/models/' + AI_TEXT_MODEL,
            {
                ...
                body: JSON.stringify({
                    inputs: 'You are QENTRA AI, an educational assistant. Answer briefly and clearly. User: ' + message,
                    parameters: {
                        max_new_tokens: 500,
                        temperature: 0.7,
                        return_full_text: false
