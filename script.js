...
function applySettings() {
    ...
    // Dark mode button - اسود لما مش مفعل
...
async function generateQuestions() {
    ...
    try {
        ...
        const prompt = `You are QENTRA AI, an educational assistant. Look at this image carefully and analyze its content deeply.

IMPORTANT RULES:
1. Generate EXACTLY 10 questions based ONLY on the scientific/educational content in the image
2. Questions must be DEEP and MEANINGFUL - ask about concepts, relationships, cause-effect, analysis, and application
3. DO NOT ask about superficial details like page numbers, formatting, colors, or layout
4. Questions should make the student THINK and understand the material deeply
5. Include questions that require connecting ideas and understanding between the lines
6. Questions 1-3: Easy (basic understanding)
7. Questions 4-7: Medium (analysis and application)
8. Questions 8-10: Hard (critical thinking and synthesis)

Return ONLY valid JSON:
{
    "success": true,
    "questions": [
        {"number": 1, "difficulty": "easy", "question": "Deep question about the content", "answer": "Clear, concise answer"},
        {"number": 2, "difficulty": "easy", "question": "Deep question about the content", "answer": "Clear, concise answer"}
    ]
}

If the image does not contain clear educational content, return:
...
// ============ Chat with Qentra ============
async function chatWithQentra(message, questionContext = null) {
    try {
        let systemPrompt = `You are QENTRA AI, an educational assistant. Your answers must be SHORT, CLEAR, and USEFUL.
        
RULES:
1. Give BRIEF answers (max 3-4 sentences)
2. Focus on helping the student understand concepts
3. Use simple language
4. If explaining a question, address it directly
...
Answer in the same language as the user.`;
        ...
        if (questionContext) {
            systemPrompt += `\n\nThe user is asking about question ${questionContext.number}: "${questionContext.question}". The answer is: "${questionContext.answer}". Explain it clearly but briefly.`;
        }
...
function displayQuestions(questions) {
    ...
    // Add event listeners for explain buttons
...
async function sendChatMessage() {
    ...
    try {
        ...
        pendingContext = null;
        const response = await chatWithQentra(message, context);
        hideTyping();
...
function init() {
    ...
    showView('home');
    console.log('QENTRA AI initialized');
}
