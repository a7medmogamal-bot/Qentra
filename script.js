...
const translations = {
    ar: {
        ...
        ask_me: 'اسألني عن أي موضوع دراسي'
    },
    en: {
        ...
        ask_me: 'Ask me about any study topic'
    }
};
...
// ============ Study Chat ============
async function chatWithQentra(message, imageBase64 = null) {
    try {
        const systemPrompt = `أنت QENTRA AI، مساعد تعليمي متخصص. أنت هنا لمساعدة الطلاب في فهم المواد الدراسية.

قواعد مهمة:
1. أجب باختصار ووضوح (3-4 جمل كحد أقصى)
2. ركز على المساعدة في الفهم وليس إعطاء الإجابات فقط
3. استخدم لغة بسيطة ومناسبة للمستوى التعليمي
4. إذا كان السؤال عن موضوع دراسي، اشرحه بشكل مبسط
5. كن مشجعاً ومحفزاً للطالب
...
أجب بنفس لغة المستخدم.`;
        ...
        if (imageBase64) {
            fullPrompt = systemPrompt + '\n\nالمستخدم: ' + message + '\n\n(هناك صورة مرفقة، حللها وساعد في فهم محتواها)';
        }
...
async function generateQuestions() {
    ...
    try {
        ...
        const prompt = `أنت QENTRA AI. انظر إلى هذه الصورة بعناية وقم بتوليد 10 أسئلة تعليمية بناءً على محتواها.

القواعد:
1. الأسئلة يجب أن تكون عميقة وهادفة
2. لا تسأل عن تفاصيل سطحية
3. الأسئلة 1-3: سهلة، 4-7: متوسطة، 8-10: صعبة

أرجع JSON فقط:
{
    "success": true,
    "questions": [
        {"number": 1, "difficulty": "easy", "question": "السؤال", "answer": "الإجابة"}
    ]
}
...
{"success": false, "error": "unclear"}`;
        ...
        if (parsed.success && parsed.questions && parsed.questions.length === 10) {
            ...
            errorMsg.textContent = '✅ تم توليد 10 أسئلة بنجاح! يمكنك الآن مناقشتها مع Qentra';
            setTimeout(() => { errorMsg.style.display = 'none'; }, 4000);
        } else {
            updateDailyUsage(false);
            const left = state.dailyUsage.chancesLeft;
            showError('الصورة غير واضحة. حاول رفع صورة أوضح. لديك ', left + ' ' + t('chances_left'));
        }
    } catch (e) {
        console.error('خطأ:', e);
        hideLoading();
...
async function sendChatMessage() {
    ...
    try {
        ...
        const response = await chatWithQentra(fullMessage);
        hideTyping();
...
elements.landingBtn.addEventListener('click', () => {
    ...
    // رسالة ترحيب
    setTimeout(() => {
        addChatMessage('ai', '📚 أهلاً بك في Qentra! أنا هنا لمساعدتك في دراستك. اسألني عن أي موضوع تريد فهمه، أو ارفع صورة من مذكراتك وسأقوم بتوليد أسئلة لك.');
    }, 500);
});
...
elements.newChatBtn.addEventListener('click', () => {
    ...
    addChatMessage('ai', '📚 مرحباً! اسألني عن أي موضوع دراسي، أو ارفع صورة لتوليد أسئلة.');
});
...
window.addEventListener('beforeunload', () => {
    if (state.currentQuestions || state.chatHistory.length > 0) {
...
function init() {
    ...
    showView('home');
    console.log('QENTRA AI - Study Assistant initialized with Groq');
}
